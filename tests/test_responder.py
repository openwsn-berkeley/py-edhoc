from cose import OKP, CoseHeaderKeys
...
def test_responder_message2(responder, test_vectors):
    ...
    hash_func = config_cose(CipherSuite(responder.msg_1.selected_cipher).hash).hash
    crv = CoseEllipticCurves(CipherSuite(responder.msg_1.selected_cipher).dh_curve)

    assert responder.shared_secret(responder.ephemeral_key, OKP(x=responder.g_x, crv=crv)) == test_vectors['S']['g_xy']
...
def test_responder_finalize(responder, test_vectors):
    ...
    if CoseHeaderKeys.KID in test_vectors['I']['id_cred']:
        assert decoded[0] == EdhocMessage.encode_bstr_id(test_vectors['I']['id_cred'][CoseHeaderKeys.KID])
    else:
