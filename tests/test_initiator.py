...
@pytest.mark.parametrize("method_corr, suites, g_x, connection_id, aad, output, cred, cred_id",
                         [
...
def test_initiator_message_one(method_corr, suites, g_x, connection_id, aad, output, cred, cred_id):
    itr = Initiator(auth_method=0, corr=method_corr, connection_id=connection_id, cred=cred, cred_id=cred_id)
    itr.priv_key = OKP(x=g_x)
    assert output == itr.create_message_one()


@pytest.mark.parametrize("input1, input2, output, cred, cred_id",
                         [
                             (unhexlify('01005820898ff79a02067a16ea1eccb90fa52246f5aa4dd6ec076bba0259d904b7ec8b0c40'),
...
def test_initiator_message_transcript(input1, input2, output, cred, cred_id):
    itr = Initiator(auth_method=0, corr=0, connection_id=os.urandom(12), cred=cred, cred_id=cred_id)
    itr.cipher_suite = 0
